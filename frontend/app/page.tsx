...
// Google OAuth redirect 登录
function googleRedirectLogin(returnTo?: string) {
  if (returnTo) sessionStorage.setItem('auth_return_to', returnTo);
  const clientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || '';
  const redirectUri = encodeURIComponent(`${window.location.origin}/auth/callback`);
  const scope = encodeURIComponent('openid email profile');
  const url = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${redirectUri}&response_type=id_token&scope=${scope}&nonce=${Math.random().toString(36).slice(2)}`;
      ...
      <nav className="fixed top-0 left-0 right-0 bg-white/90 backdrop-blur-sm border-b border-gray-100 z-40">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/en" className="text-xs border border-gray-200 rounded-lg px-2 py-1 text-gray-500 hover:border-gray-400 transition flex items-center gap-1">
              🌐 EN
            </Link>
            {authLoading ? (
              ...
              <button
                onClick={() => googleRedirectLogin(window.location.pathname)}
                className="flex items-center gap-2 bg-white border border-gray-300 hover:border-gray-400 hover:shadow-sm text-gray-700 text-sm font-medium px-4 py-2 rounded-lg transition"
              >
            ...
            {!authLoading && !user && (
              <div className="w-full max-w-xl border-2 border-dashed border-gray-200 rounded-2xl p-12 flex flex-col items-center bg-white/80">
                ...
                <button
                  ...
                  className="flex items-center gap-3 bg-white border-2 border-gray-200 hover:border-blue-400 hover:shadow-md text-gray-700 font-semibold px-6 py-3 rounded-xl transition"
                >
